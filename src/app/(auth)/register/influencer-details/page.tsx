...
import { DropdownOption } from '@/_Components/ui/DropDown';
...
export default function InfluencerDetailsPage() {
  ...
  const ageOptions: DropdownOption[] = Array.from({ length: 70 }, (_, i) => i + 18).map(year => ({ // <-- وهنا أيضاً
    value: `${year} Y`,
    label: `${year} Y`
  }));
  ...
  return (
    <div className="flex flex-col items-center  p-4 text-secondary">
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-8 mt-4">
        <Back />
        <h1 className="text-xl font-medium text-center flex-grow">Influencers</h1>
      </div>
      ...
      <div className="flex flex-col w-full max-w-md px-5 pb-8">
        <form onSubmit={handleSubmit} className="w-full space-y-6">
          ...
          <DropDown
            ...
            placeholder="Male"
            error={errorMessage.includes('gender') ? errorMessage : undefined}
          />
          ...
          <DropDown
            ...
            options={ageOptions}
            placeholder="20 Y"
            error={errorMessage.includes('age') ? errorMessage : undefined} // تمرير رسالة الخطأ المناسبة
          />
          ...
          {/* Do you have time to enter more details Section (Collapsible) */}
              {!showMoreDetails && (
                ...
                <button
                  ...
                  className="w-full flex justify-between items-center text-place font-nomral py-2"
                >
                  ...
                  <div className="space-y-3">
                    {socialMediaPlatforms.map((platform) => (
                      <div key={platform.name} className="flex items-center justify-between">
                        {/* استخدام CheckBox الجديد مع تمرير الـ label جوه الـ props */}
                        <Checkbox
                          id={platform.name}
                          label={platform.name}
                          name="platform"
