...
export default function WalletPage() {
    ...
    // حالة لإظهار/إخفاء الـ popup
    const [showWithdrawModal, setShowWithdrawModal] = useState(false);
    // حالة لمبلغ السحب في الـ popup
...
const withdrawalRequests = mockTransactions.filter(
  ...
  // فلترة المعاملات العادية (جميع المعاملات عدا السحب قيد المعالجة)
const regularTransactions = mockTransactions.filter(
    (t) => !(t.type === "withdrawal" && t.status === "pending")
);

  // دالة لمعالجة فتح الـ popup
const handleOpenWithdrawModal = () => {
    setShowWithdrawModal(true);
    setWithdrawAmount(""); // مسح أي مبلغ سابق
    setWithdrawError("");
};

  // دالة لمعالجة إغلاق الـ popup
const handleCloseWithdrawModal = () => {
    setShowWithdrawModal(false);
};

  // دالة لمعالجة طلب السحب من الـ popup
const handleConfirmWithdraw = (e: FormEvent) => {
    ...
    setTimeout(() => {
      setCurrentBalance((prevBalance) => prevBalance - amount); // تحديث الرصيد
        ...
        // إضافة معاملة سحب جديدة (mock)
        id: `trans-${mockTransactions.length + 1}`,
        userId: "user-3", // نفترض هذا هو المستخدم الحالي
        amount: amount,
        type: "withdrawal",
        date: new Date().toLocaleDateString("en-GB"), // تاريخ اليوم
        status: "completed", // أو 'pending' إذا كنت تريد محاكاة المعالجة
...
return (
    <div className="p-4 text-secondary">
        ...
        <div className="w-full max-w-md flex justify-between items-center mb-8 mt-4">
            <Back />
            <h1 className="text-xl font-medium text-center flex-grow md:hidden">Wallet</h1>
        </div>
